...
with tab2:
    ...
    if st.button(f"🧠 让 [{selected_model}] 精选 Top 3 立项组合", type="primary"):
        ...
        prompt = f"""【🔴 团队真实复盘记忆库 (正负双向标尺)】：\n{rules_text}\n\n请评估以下 15 个随机立项组合（括号内为底层机制，可能包含生涩词汇）：\n{chr(10).join(combos)}\n\n【强制双向动态打分规则】：\n1. 👎 负向惩罚（防暴死）：提取新方案商业逻辑，比对复盘库中的‘👎死穴’。命中 1 次提示，2 次大幅下调权重，3 次及以上直接淘汰。\n2. 👍 正向奖励（抓红利）：比对复盘库中的‘👍优势’。若新方案巧妙复刻了历史成功机制，必须给予极高的创新与留存加分，优先入选！\n\n【⚠️ 行业黑话与对标翻译指令（极度重要）】：\n你输出的 `combo` 字段绝对不能照抄生涩的学术词汇（如“滑动战术叙事”、“社交破坏”、“拟真金融”等）！你必须像国内资深游戏制作人一样，将它们翻译成具体的研发黑话和爆款对标。\n- “拟真金融”必须翻译为具体的题材，如“现代模拟商战题材”。\n- “滑动叙事/操作”必须翻译为具体的微操，如“左右划卡(类Reigns)”、“屏幕拖拽/二合解谜”。\n- “社交仇恨链”必须翻译为“类《Coin Master/大富翁》偷菜社交”。\n\n强制格式：[具体题材]题材 + [具体入局玩法(如二合/拖拽/割草/找茬)]入局 + [具体画风]画风 + [中后期循环/知名对标(如类似《无尽冬日》SLG框架)]\n正向示例：丧尸末日题材 + 列车模拟经营入局 + 欧美3D卡通画风 + 类无尽冬日的SLG中后期循环。\n反面示例（绝对禁止输出此类）：[拟真金融博弈] + [滑动战术叙事] + [高保真3D] + [社交破坏驱动]\n\n淘汰掉 12 个废案，选出最具爆发力且最能踩中成功杠杆的 3 个。返回纯JSON数组：[ {{"id":"1", "idea_name":"代号", "combo":"(必须按照上述黑话公式重写并翻译的立项组合)", "evaluation":"150字内双向分析：说明踩中了哪些历史优势加分，或死穴扣分"}} ]"""
        ...
        for idx, obj in enumerate(st.session_state.top_ideas):
            with st.container(border=True):
                ...
                if st.button(f"📝 深度推演：生成【{obj.get('idea_name')}】详案", key=f"btn_{idx}"):
                    ...
                    detail_prompt = f"""【🔴 团队真实复盘记忆库 (正负双向标尺)】：\n{rules_text}\n\n基于以上记忆库和【双向动态奖惩机制】，为组合 [{obj.get('combo')}] 撰写详尽 Markdown 立项案。必须严格按照以下结构输出：\n\n⚠️ 警告：企划案全文必须使用游戏圈一线“大白话”和“研发黑话”，多用知名竞品作为对标解释玩法（如：类《吸血鬼幸存者》割草、类《咸鱼之王》开箱、类《无尽冬日》SLG）。绝对禁止使用“战术叙事”、“拟真博弈”等装腔作势的学术词汇！\n\n### 💡 一句话高概念 (用“对标竞品A + 题材B”的句式，让人一秒听懂)\n### 📊 综合商业成功率预判：[XX]/100分\n\n#### 1. 获客与留存风险评估 (权重 40%)：[XX]/40分\n* **玩法降维与过渡拆解：** 必须用接地气的行话，说明轻度入局玩法（如二合/割草）是如何平滑过渡到重度中后期循环（如SLG/卡牌）的。\n* **👍 历史优势复刻 (正向加分)：** 明确列出该方案成功化用了记忆库中哪几个项目的【👍优势】（必须写出具体复盘项目名作为证据）。触发了多少额外加分？若无则写“未踩中已知历史优势”。\n\n#### 2. 长线变现健康度评估 (权重 60%)：[XX]/60分\n* **商业化模型落地：** 说明是偏向轻度混合变现（IAA+IAP）还是重度内购大R深坑。\n* **👎 历史雷区碰撞 (负向惩罚)：** 明确列出该方案踩中了记忆库中哪几个项目的【👎死穴】（写出具体项目名）。触发了何种扣分惩罚（1次黄牌，2次橙警扣10-15分，3次红警扣30分以上）？若无则写“完美避开已知历史雷区”。\n\n#### 🚀 核心破局建议：\n基于记忆库的【🧠法则】，给出扬长避短的具体落地执行方向（如：买量素材怎么打、前期卡数值节奏等）。"""
